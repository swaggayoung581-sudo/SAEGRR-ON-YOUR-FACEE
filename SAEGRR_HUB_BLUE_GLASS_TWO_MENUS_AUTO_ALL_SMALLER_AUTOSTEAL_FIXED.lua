...
do
    ...
    -- Wide/landscape rectangle like the reference, but kept compact enough
    -- to fit smaller screens. The contents remain inside the same Menu 1.
    local camera = workspace.CurrentCamera
    local function getMainSize()
        local vp = camera and camera.ViewportSize or Vector2.new(800, 600)
        return math.max(300, math.min(480, vp.X - 20)),
               math.max(260, math.min(320, vp.Y - 40))
    end
    ...
    -- Keep the rectangle centered and responsive after rotation/resize.
    if camera then
    ...
    stroke.Color = Color3.fromRGB(30, 160, 255)

    local gradient = Instance.new("UIGradient", stroke)
    gradient.Rotation = 0
    gradient.Color = ColorSequence.new(Color3.fromRGB(30, 160, 255))
    ...
    glow.Color = Color3.fromRGB(30, 160, 255)
    ...
    made.Size = UDim2.new(1, -20, 0, 14)
    made.Position = UDim2.fromOffset(12, 32)
    ...
    -- Left-side category rail, like the reference's separate menu doors.
    local side = Instance.new("Frame", main)
    side.Name = "CategoryRail"
    side.Size = UDim2.new(0, 92, 1, -60)
    side.Position = UDim2.fromOffset(8, 52)
    side.BackgroundColor3 = Color3.fromRGB(8, 4, 5)
    ...
    sideStroke.Color = Color3.fromRGB(10, 95, 180)
    ...
    scroll.Size = UDim2.new(1, -116, 1, -60)
    scroll.Position = UDim2.fromOffset(108, 52)
    ...
    scrollStroke.Color = Color3.fromRGB(10, 95, 180)
    ...
    local function section(text)
        ...
        l.Text = text
        l.TextColor3 = Color3.fromRGB(70, 205, 255)
        l.TextSize = 11
        l.TextXAlignment = Enum.TextXAlignment.Left
        sectionRefs[string.upper(text)] = l
        return l
    end
    ...
    local function button(text, callback, h)
        ...
        b.BackgroundColor3 = Color3.fromRGB(5, 30, 58)
        b.BorderSizePixel = 0
    ...
    local function toggle(text, callback)
        ...
        b = button(text .. " [OFF]", function()
            ...
            b.BackgroundColor3 = on and Color3.fromRGB(10, 120, 220) or Color3.fromRGB(5, 30, 58)
    ...
    local navButtons = {}
    local function addNav(label, targetKey, order)
        local b = Instance.new("TextButton", side)
        b.LayoutOrder = order
        b.Size = UDim2.new(1, 0, 0, 48)
        b.BackgroundColor3 = Color3.fromRGB(5, 28, 55)
        b.BorderSizePixel = 0
        b.Font = Enum.Font.GothamBold
        b.Text = label
        b.TextColor3 = Color3.fromRGB(255, 255, 255)
        ...
        st.Thickness = 1
        st.Color = Color3.fromRGB(95, 8, 16)
        navButtons[targetKey] = b
        b.MouseButton1Click:Connect(function()
            local target = sectionRefs[targetKey]
            if target and target.Parent then
                local y = math.max(0, target.AbsolutePosition.Y - scroll.AbsolutePosition.Y + scroll.CanvasPosition.Y - 4)
                scroll.CanvasPosition = Vector2.new(0, y)
            end
            for key, btn in pairs(navButtons) do
                btn.BackgroundColor3 = (key == targetKey) and Color3.fromRGB(125, 10, 22) or Color3.fromRGB(5, 28, 55)
            end
        end)
        return b
    end

    -- Create the category buttons now; targets are resolved after the sections exist.
    addNav("EGG\nFILTERS\nAREA • RARITY", "EGG FILTERS", 1)
    addNav("AUTO FARM\nAUTO STEAL", "AUTO FARM", 2)
    addNav("ABYSS\nOVERLOAD\nAUTO ENTER • FIGHT", "ABYSS OVERLORD", 3)
    addNav("ESP\nEGG • PLOT\nEGG ESP NOW", "ESP", 4)
    ...
    tpSpeedRow.BackgroundColor3 = Color3.fromRGB(5, 28, 55)
    ...
    tpMinus.BackgroundColor3 = Color3.fromRGB(5, 55, 105)
    ...
    tpPlus.BackgroundColor3 = Color3.fromRGB(10, 120, 220)
    ...
    -- ---------- ESP ----------
    section("ESP")
    local eggESP = BX.require("features.esp.eggs")
    local plotESP = BX.require("features.esp.plot")

    toggle("EGG ESP", function(on)
        local ok, why = eggESP.setEnabled(on)
        if ok == false then error(why or "Could not change Egg ESP") end
        say(on and "Egg ESP: ON" or "Egg ESP: OFF")
    end)

    toggle("PLOT ESP", function(on)
        local ok, why = plotESP.setEnabled(on)
        if ok == false then error(why or "Could not change Plot ESP") end
        say(on and "Plot ESP: ON" or "Plot ESP: OFF")
    end)

    button("EGG ESP NOW", function()
        local ok, why = eggESP.setEnabled(true)
        if ok == false then
            say("Egg ESP error: " .. tostring(why))
            return
        end
        say("Egg ESP: ON NOW")
    end, 30)

    if navButtons["EGG FILTERS"] then
        navButtons["EGG FILTERS"].BackgroundColor3 = Color3.fromRGB(125, 10, 22)
    end

    --------------------------------------------------------------------------
    -- MENU 2: BLUE GLASS EGG TELEPORTER
    -- Portrait/mobile layout inspired by the supplied SAE HUB reference.
    --------------------------------------------------------------------------
    local eggsFeature = BX.require("features.eggs")

    local second = Instance.new("Frame", gui)
    second.Name = "SAEGRR_AutoStealPanel"
    second.Size = UDim2.fromOffset(225, 325)
    second.Position = UDim2.new(0.5, 118, 0.5, -162)
    second.BackgroundColor3 = Color3.fromRGB(4, 18, 38)
    second.BackgroundTransparency = 0.10
    second.BorderSizePixel = 0
    second.Active = true
    second.Draggable = true
    second.ZIndex = 20
    Instance.new("UICorner", second).CornerRadius = UDim.new(0, 18)

    local secondStroke = Instance.new("UIStroke", second)
    secondStroke.Thickness = 2.5
    secondStroke.Color = Color3.fromRGB(30, 160, 255)
    secondStroke.ZIndex = 20

    local secondGlow = Instance.new("UIStroke", second)
    secondGlow.Thickness = 9
    secondGlow.Transparency = 0.72
    secondGlow.Color = Color3.fromRGB(0, 110, 255)
    secondGlow.ZIndex = 19

    local secondGradient = Instance.new("UIGradient", secondStroke)
    secondGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0.00, Color3.fromRGB(0, 70, 180)),
        ColorSequenceKeypoint.new(0.25, Color3.fromRGB(40, 190, 255)),
        ColorSequenceKeypoint.new(0.50, Color3.fromRGB(220, 250, 255)),
        ColorSequenceKeypoint.new(0.75, Color3.fromRGB(30, 130, 255)),
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB(0, 55, 150))
    })
    task.spawn(function()
        local t = 0
        while gui.Parent and second.Parent do
            local dt = RunService.RenderStepped:Wait()
            t = t + dt
            secondGradient.Rotation = (t * 55) % 360
            local pulse = math.sin(t * 2) * 0.5 + 0.5
            secondGlow.Transparency = 0.78 - pulse * 0.12
        end
    end)

    local secondTitle = Instance.new("TextLabel", second)
    secondTitle.Size = UDim2.new(1, -48, 0, 26)
    secondTitle.Position = UDim2.fromOffset(9, 5)
    secondTitle.BackgroundTransparency = 1
    secondTitle.Font = Enum.Font.GothamBlack
    secondTitle.Text = "SAE HUB"
    secondTitle.TextColor3 = Color3.fromRGB(245, 252, 255)
    secondTitle.TextSize = 17
    secondTitle.TextXAlignment = Enum.TextXAlignment.Left
    secondTitle.ZIndex = 21

    local secondSub = Instance.new("TextLabel", second)
    secondSub.Size = UDim2.new(1, -24, 0, 13)
    secondSub.Position = UDim2.fromOffset(10, 27)
    secondSub.BackgroundTransparency = 1
    secondSub.Font = Enum.Font.GothamBold
    secondSub.Text = "EGG TELEPORTER"
    secondSub.TextColor3 = Color3.fromRGB(55, 200, 255)
    secondSub.TextSize = 6
    secondSub.TextXAlignment = Enum.TextXAlignment.Left
    secondSub.ZIndex = 21

    local secondClose = Instance.new("TextButton", second)
    secondClose.Size = UDim2.fromOffset(32, 29)
    secondClose.Position = UDim2.new(1, -40, 0, 6)
    secondClose.BackgroundColor3 = Color3.fromRGB(5, 40, 85)
    secondClose.BackgroundTransparency = 0.10
    secondClose.BorderSizePixel = 0
    secondClose.Font = Enum.Font.GothamBold
    secondClose.Text = "X"
    secondClose.TextColor3 = Color3.fromRGB(230, 250, 255)
    secondClose.TextSize = 14
    secondClose.ZIndex = 21
    Instance.new("UICorner", secondClose).CornerRadius = UDim.new(0, 12)
    local closeStroke = Instance.new("UIStroke", secondClose)
    closeStroke.Thickness = 1.3
    closeStroke.Color = Color3.fromRGB(25, 145, 255)

    local function glassRow(parent, y, h)
        local f = Instance.new("Frame", parent)
        f.Size = UDim2.new(1, -28, 0, h)
        f.Position = UDim2.fromOffset(14, y)
        f.BackgroundColor3 = Color3.fromRGB(7, 42, 82)
        ...
        s.Thickness = 1.2
        s.Color = Color3.fromRGB(30, 150, 255)
        s.Transparency = 0.15
        return f
    end

    local selectedText = glassRow(second, 45, 47)
    local selectedHeader = Instance.new("TextLabel", selectedText)
    selectedHeader.Size = UDim2.new(1, -12, 0, 14)
    selectedHeader.Position = UDim2.fromOffset(6, 4)
    selectedHeader.BackgroundTransparency = 1
    selectedHeader.Font = Enum.Font.GothamBold
    selectedHeader.Text = "◉  SELECT EGG TO TELEPORT"
    selectedHeader.TextColor3 = Color3.fromRGB(70, 205, 255)
    selectedHeader.TextSize = 7
    selectedHeader.TextXAlignment = Enum.TextXAlignment.Left
    selectedHeader.ZIndex = 22

    local selectedName = Instance.new("TextLabel", selectedText)
    selectedName.Size = UDim2.new(1, -12, 0, 22)
    selectedName.Position = UDim2.fromOffset(6, 20)
    selectedName.BackgroundColor3 = Color3.fromRGB(8, 52, 100)
    selectedName.BackgroundTransparency = 0.08
    selectedName.BorderSizePixel = 0
    selectedName.Font = Enum.Font.GothamBold
    selectedName.Text = "NONE SELECTED"
    selectedName.TextColor3 = Color3.fromRGB(245, 252, 255)
    selectedName.TextSize = 8
    selectedName.TextXAlignment = Enum.TextXAlignment.Left
    selectedName.TextTruncate = Enum.TextTruncate.AtEnd
    selectedName.ZIndex = 22
    Instance.new("UICorner", selectedName).CornerRadius = UDim.new(0, 9)

    local autoFindButton = Instance.new("TextButton", second)
    autoFindButton.Size = UDim2.new(1, -16, 0, 21)
    autoFindButton.Position = UDim2.fromOffset(8, 98)
    autoFindButton.BackgroundColor3 = Color3.fromRGB(8, 80, 150)
    autoFindButton.BorderSizePixel = 0
    autoFindButton.Font = Enum.Font.GothamBold
    autoFindButton.Text = "AUTO FIND • M/S + KG"
    autoFindButton.TextColor3 = Color3.fromRGB(245, 252, 255)
    autoFindButton.TextSize = 7
    autoFindButton.ZIndex = 21
    Instance.new("UICorner", autoFindButton).CornerRadius = UDim.new(0, 9)
    local autoFindStroke = Instance.new("UIStroke", autoFindButton)
    autoFindStroke.Thickness = 1
    autoFindStroke.Color = Color3.fromRGB(50, 190, 255)

    -- TP SPEED
    local secondTPSpeedRow = glassRow(second, 126, 31)
    local secondTPSpeedLabel = Instance.new("TextLabel", secondTPSpeedRow)
    secondTPSpeedLabel.Size = UDim2.new(1, -84, 1, 0)
    secondTPSpeedLabel.Position = UDim2.fromOffset(6, 0)
    secondTPSpeedLabel.BackgroundTransparency = 1
    secondTPSpeedLabel.Font = Enum.Font.GothamBold
    secondTPSpeedLabel.TextColor3 = Color3.fromRGB(235, 250, 255)
    secondTPSpeedLabel.TextSize = 7
    ...
    secondTPMinus.Size = UDim2.fromOffset(22, 22)
    secondTPMinus.Position = UDim2.new(1, -51, 0.5, -11)
    secondTPMinus.BackgroundColor3 = Color3.fromRGB(5, 50, 95)
    ...
    Instance.new("UICorner", secondTPMinus).CornerRadius = UDim.new(0, 8)

    local secondTPPlus = Instance.new("TextButton", secondTPSpeedRow)
    secondTPPlus.Size = UDim2.fromOffset(22, 22)
    secondTPPlus.Position = UDim2.new(1, -25, 0.5, -11)
    secondTPPlus.BackgroundColor3 = Color3.fromRGB(10, 120, 230)
    ...
    Instance.new("UICorner", secondTPPlus).CornerRadius = UDim.new(0, 8)

    local function updateSecondTPSpeedVisual()
    ...
    -- AUTO STEAL
    local autoStealRow = glassRow(second, 165, 34)
    local autoStealButton = Instance.new("TextButton", autoStealRow)
    autoStealButton.Size = UDim2.new(1, -76, 1, -6)
    autoStealButton.Position = UDim2.fromOffset(5, 3)
    autoStealButton.BackgroundTransparency = 1
    autoStealButton.BorderSizePixel = 0
    autoStealButton.Font = Enum.Font.GothamBold
    autoStealButton.Text = "🏃  AUTO STEAL TP"
    autoStealButton.TextColor3 = Color3.fromRGB(245, 252, 255)
    autoStealButton.TextSize = 7
    autoStealButton.TextXAlignment = Enum.TextXAlignment.Left
    autoStealButton.ZIndex = 22

    local autoStealStatus = Instance.new("TextLabel", autoStealRow)
    autoStealStatus.Size = UDim2.fromOffset(64, 12)
    autoStealStatus.Position = UDim2.new(1, -69, 0, 2)
    autoStealStatus.BackgroundTransparency = 1
    autoStealStatus.Font = Enum.Font.Gotham
    autoStealStatus.Text = "OFF"
    autoStealStatus.TextColor3 = Color3.fromRGB(120, 190, 220)
    autoStealStatus.TextSize = 6
    autoStealStatus.TextXAlignment = Enum.TextXAlignment.Right
    autoStealStatus.ZIndex = 22

    local autoStealOn = false
    local function updateAutoStealButton()
        autoStealStatus.Text = autoStealOn and "ON" or "OFF"
        autoStealStatus.TextColor3 = autoStealOn and Color3.fromRGB(100, 235, 255) or Color3.fromRGB(120, 190, 220)
    end

    -- Scrollable egg list: keeps the reference's stacked egg-card look.
    local eggList = Instance.new("ScrollingFrame", second)
    eggList.Name = "EggView"
    eggList.Size = UDim2.new(1, -16, 0, 78)
    eggList.Position = UDim2.fromOffset(8, 207)
    eggList.BackgroundColor3 = Color3.fromRGB(3, 22, 43)
    eggList.BackgroundTransparency = 0.18
    eggList.BorderSizePixel = 0
    eggList.ScrollBarThickness = 3
    eggList.ScrollBarImageColor3 = Color3.fromRGB(70, 205, 255)
    eggList.AutomaticCanvasSize = Enum.AutomaticSize.Y
    eggList.CanvasSize = UDim2.fromOffset(0, 0)
    eggList.ZIndex = 21
    Instance.new("UICorner", eggList).CornerRadius = UDim.new(0, 12)
    local eggListStroke = Instance.new("UIStroke", eggList)
    eggListStroke.Thickness = 1
    eggListStroke.Color = Color3.fromRGB(25, 130, 230)
    eggListStroke.Transparency = 0.10
    ...
    eggPadding.PaddingTop = UDim.new(0, 3)
    eggPadding.PaddingLeft = UDim.new(0, 3)
    eggPadding.PaddingRight = UDim.new(0, 3)
    eggPadding.PaddingBottom = UDim.new(0, 3)

    -- Status line showing the current auto-find mode.
    local findStatus = Instance.new("TextLabel", second)
    findStatus.Size = UDim2.new(1, -16, 0, 12)
    findStatus.Position = UDim2.fromOffset(8, 291)
    findStatus.BackgroundTransparency = 1
    findStatus.Font = Enum.Font.Gotham
    findStatus.Text = "AUTO FIND: OFF"
    findStatus.TextColor3 = Color3.fromRGB(80, 190, 235)
    findStatus.TextSize = 6
    findStatus.TextXAlignment = Enum.TextXAlignment.Left
    findStatus.ZIndex = 22

    local stopButton = Instance.new("TextButton", second)
    stopButton.Size = UDim2.new(1, -16, 0, 22)
    stopButton.Position = UDim2.fromOffset(8, 302)
    stopButton.BackgroundColor3 = Color3.fromRGB(7, 55, 105)
    stopButton.BorderSizePixel = 0
    stopButton.Font = Enum.Font.GothamBold
    stopButton.Text = "STOP"
    stopButton.TextColor3 = Color3.fromRGB(245, 252, 255)
    stopButton.TextSize = 8
    stopButton.ZIndex = 22
    Instance.new("UICorner", stopButton).CornerRadius = UDim.new(0, 10)

    local autoFindOn = false
    local selectedUid = nil
    ...
    local function selectEgg(e)
        ...
        selectedName.Text = tostring(e.name or "Unknown Egg")
            .. "  [" .. tostring(e.rarity or "?") .. "]  •  "
            .. string.format("%.1fkg", tonumber(e.kg) or 0)
        auto.setOptions("farm", {
            uid = selectedUid,
            continuous = true
        })
    ...
    local function refreshEggView()
        ...
        if #list == 0 then
            ...
            empty.TextColor3 = Color3.fromRGB(100, 190, 230)
            empty.TextSize = 9
            empty.ZIndex = 22
            return
        end

        -- Keep highest M/S first; use KG as the tie-breaker.
        ...
        for index, e in ipairs(list) do
            ...
            b.BackgroundColor3 = (selectedUid == tostring(e.uid))
                and Color3.fromRGB(15, 115, 210)
                or Color3.fromRGB(7, 47, 90)
            b.BackgroundTransparency = 0.10
            b.BorderSizePixel = 0
            b.Font = Enum.Font.GothamBold
            b.Text = describeEgg(e)
            b.TextColor3 = Color3.fromRGB(245, 252, 255)
            b.TextSize = 6
            ...
            bs.Thickness = 0.8
            bs.Color = Color3.fromRGB(30, 145, 240)
    ...
    local function autoFindBest()
        local list = getEggList()
        if #list == 0 then
            findStatus.Text = "AUTO FIND: NO EGGS FOUND"
            return false
        end
        table.sort(list, function(a, b)
            local av, bv = tonumber(a.value) or 0, tonumber(b.value) or 0
        ...
        selectEgg(list[1])
        autoFindOn = true
        findStatus.Text = "AUTO FIND: ON • BEST M/S + KG"
        refreshEggView()
        return true
    end

    autoFindButton.MouseButton1Click:Connect(function()
        autoFindBest()
    end)
    ...
    task.spawn(function()
        while gui.Parent and second.Parent do
            task.wait(1.5)
            if autoFindOn and not autoStealOn then
                autoFindBest()
            elseif autoFindOn and autoStealOn then
                local list = getEggList()
                local found = false
                for _, e in ipairs(list) do
                    if tostring(e.uid) == tostring(selectedUid) then
                ...
                if not found then
                    autoFindBest()
                    if selectedUid then
                        auto.setOptions("farm", {uid = selectedUid, continuous = true})
                    end
                end
            end
        end
    ...
    autoStealButton.MouseButton1Click:Connect(function()
        if not autoStealOn then
            -- If no target was selected, automatically pick the highest M/S
            -- target (KG is the tie-breaker) instead of refusing to start.
            if not selectedUid then
                if not autoFindBest() then
                    findStatus.Text = "AUTO STEAL: NO TARGET FOUND"
                    return
                end
            end
            ...
            -- Make the Farm tab the single owner of the same AutoSteal engine.
            -- This prevents a previous Main-tab owner from making this button
            -- appear ON while the worker is not actually running for Farm.
            auto.stop()
            task.wait()
            ...
            if ok == false or not auto.isRunning() then
                setSecondAutoVisual(false)
                syncMainAutoVisual(false)
                findStatus.Text = "AUTO STEAL ERROR: " .. tostring(why or "worker did not start")
                return
            end

            setSecondAutoVisual(true)
            syncMainAutoVisual(true)
            findStatus.Text = "AUTO STEAL: ON • TARGET LOCKED"
        else
            auto.setEnabled(false, "farm")
            setSecondAutoVisual(false)
            syncMainAutoVisual(false)
            findStatus.Text = "AUTO STEAL: OFF"
        end
    end)
    ...
    ms.Thickness = 2.2
    ms.Transparency = 0
    ms.Color = Color3.fromRGB(30, 160, 255)
    local mg = Instance.new("UIGradient", ms)
    mg.Rotation = 0
    mg.Color = ColorSequence.new(Color3.fromRGB(30, 160, 255))
